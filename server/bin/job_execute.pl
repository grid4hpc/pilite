...
my $fdlog;
    ...
    if ((not defined $full_job_dir) or (not length $full_job_dir) or (not defined $short_job_filename) or (not length $short_job_filename)) {
    ...
    system('touch '.$full_job_dir.'/'.$job_id.'_running');
            ...
            if ((exists $task_definition->{input_files}) and (defined $task_definition->{input_files}) and (ref $task_definition->{input_files} eq 'HASH')) {
                ...
                foreach my $input_file_name (keys %{$input_files}) {
                    ...
                    if (-e $input_file) {
                        ...
                        print STDERR "ERROR: input file '".$full_job_dir.'/'.$input_file."' for task '".$task->{id}."' is not found\n";
                        exit 1;
                    }
                }
            }
    ...
    ### !!! This is for test purposes only
    system('sleep 10');
    system('rm -rf '.$full_job_dir.'/'.$job_id.'_running');
    system('touch '.$full_job_dir.'/'.$job_id.'_finished');
